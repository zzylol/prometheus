...
func (ng *Engine) execSketch(ctx context.Context, q *query) (v parser.Value, ws storage.Warnings, err error) {
	...
	switch s := q.Statement().(type) {
	case *parser.EvalStmt:
		return ng.execEvalStmtSketch(ctx, q, s)
	case parser.TestStmt:
		return nil, nil, s(ctx)
	}
...
// execEvalStmt evaluates the expression of an evaluation statement for the given time range.
func (ng *Engine) execEvalStmtSketch(ctx context.Context, query *query, s *parser.EvalStmt) (parser.Value, storage.Warnings, error) {
	prepareSpanTimer, ctxPrepare := query.stats.GetSpanTimer(ctx, stats.QueryPreparationTime, ng.metrics.queryPrepareTime)
	mint, maxt := ng.findMinMaxTime(s)
	querier, err := query.queryable.Querier(ctxPrepare, mint, maxt)
	if err != nil {
	...
	if s.Start == s.End && s.Interval == 0 {
		...
		val, warnings, err := evaluator.EvalSketch(s.Expr)
		...
		case String:
			return result, warnings, nil
		default:
			panic(fmt.Errorf("promql.Engine.execSketch: invalid expression type %q", val.Type()))
		}
...
func (ev *evaluator) EvalSketch(expr parser.Expr) (v parser.Value, ws storage.Warnings, err error) {
	defer ev.recover(expr, &ws, &err)

	v, ws = ev.evalSketch(expr)
	return v, ws, nil
}
...
// eval evaluates the given expression as the given AST expression node requires with Sketch data structures.
func (ev *evaluator) evalSketch(expr parser.Expr) (parser.Value, storage.Warnings) {
	// This is the top-level evaluation method.
	// Thus, we check for timeout/cancellation here.
	if err := contextDone(ev.ctx, "expression evaluation"); err != nil {
		ev.error(err)
